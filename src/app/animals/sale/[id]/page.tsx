...
export default function SaleID({params}: {params: {id: string}}){
    ...
    useEffect(() => {
        ...
        async function sendPayment(){
            let email = localStorage.getItem('email');
            if (email === listing.owner_email){
                alert("You can't buy your own animal");
                setVisible2(false);
                return;
            }
            let isAdmin = localStorage.getItem('isAdmin');
            if (!email){
                alert("You need to be logged in to buy an animal");
                setVisible2(false);
                return;
            }
            if(isAdmin === 'true'){
                alert('You are not allowed to buy an animal as admin');
                setVisible2(false);
                setReply('');
                return;
            }
            setLoading(false);
            const confirmed = window.confirm('Are you sure you want to buy this animal?');
            if (!confirmed) {
                setVisible2(false);
                return;
            }
            ...
            const data = {
                "email": email,
                "amount": listing.animal_price,
            }
            console.log("data", data);
            await axios.post(`https://gqt5g3f1h4.execute-api.eu-north-1.amazonaws.com/v1/payments`,data,{
                headers: {
                    Authorizer: `${token3}`,
                }
            })
            .then((response) => {
                ...
                if (response.data.statusCode === 200){
                    ...
                    if (toast.current) {
                        console.log("toast");
                        toast.current.show({severity:'success', summary: 'Payment successful', detail:'Payment successful! Check your email for more information', life: 5000});
                        
                    }   
                    setTimeout(() => {
                       setVisible2(false);
                       //alert("Payment successful! Check you email for more information")
                    }, 4500);   
                }
                    ...
                    if (toast.current) {
                        console.log("toast");
                        toast.current.show({severity:'error', summary: 'Payment failed', detail:'Your payment has failed', life: 5000});
                    }   
                    setTimeout(() => {
                        setVisible2(false);
                     }, 2500);   
        ...
        async function sendNotification(){
            ...
            const data = {
                "to_list": [email2],
                "subject": "Animal bought successfully",
                "message": `The purchase of animal: ${listing.animal_name} has been successful. You can contact the owner of the animal at: ${listing.owner_email} or wait to be contacted!`
            }
            try {
                // Make the API call using axios
            ...
            const data2 = {
                ...
                "subject": "Your animal was bought",
                "message": `Your animal: ${listing.animal_name} has been bought. You can contact the buyer at: ${email2} or wait to be contacted!`
            }
            try {
                // Make the API call using axios
        ...
        async function deletePublication(){
            ...
            axios.delete(`https://gqt5g3f1h4.execute-api.eu-north-1.amazonaws.com/v1/listings/${listing.listing_id}`, {
                headers: {
        ...
        async function sendNot(animalName: string){
            const token3 = localStorage.getItem('access_token');
            const data = {
                "to_list": [listing.owner_email],
                "subject": "Publication Deleted",
                "message": `Your publication of animal: ${animalName}, has been Deleted because it was bought!`
            }
            try {
                // Make the API call using axios
                const response = await axios.post('https://gqt5g3f1h4.execute-api.eu-north-1.amazonaws.com/v1/notifications', data, {
    ...
    return (
        ...
        <p>Loading...</p>
            ...
            <Layout className='flex items-center justify-center flex-col text-center'>
                ...
                <div className='lg:w-2/3 mt-2 bg-black md:1/3 xs:1/5'>
                    <table className="table-auto border-separate border border-slate-500 bg-white w-full hover:scale-105">
                        ...
                        <tbody>
                            ...
                            <td className="border border-slate-700 p-2">{listing.animal_price}</td>
                ...
                {/* <div className='w-2/3 bg-white grid grid-cols-8 gap-2'>
                    ...
                    <div className='w-2/3 mt-3 text-start grid grid-cols-2 gap-2'>
                        ...
                        <div className='w-full text-end'>
                            <Button label="Buy Animal" icon="pi pi-shopping-cart" 
                            onClick={() => {
                                setVisible2(true);
                                }}
                            className='p-1 bg-gray-500 text-white hover:bg-gray-300 hover:text-black hover:border-2 hover:border-black' text raised/>
                            { loading ? (
                                <>
                                    <Dialog header="Buy Animal" visible={visible2} style={{ width: '50vw' }} onHide={() => setVisible2(false)}>
                                    <Toast ref={toast} />
                                    <div className='w-full text-center grid grid-cols-2'>
                                        <div className='w-full'>
                                            <button type="button" onClick={() => {
                                                sendPayment(); // Assuming this function handles payment logic
                                ...
                                <>
                                    <Dialog header="Verifying Payment" visible={visible2} style={{ width: '50vw' }} onHide={() => setVisible2(false)}>
                                    <Toast ref={toast} />
